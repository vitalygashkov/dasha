...
const appendUtils = (element) => {
  ...
  if (Array.isArray(element)) {
    ...
    element.getChild = (name) =>
      appendUtils(element.children.find((item) => item.tagName === name));
...
const parseBaseUrls = (url, mpd, period, representation) => {
  ...
  return {
    ...
    period: periodBaseUrl,
    representation: representationBaseUrl,
  };
};
...
const parseManifest = async (text, url, fallbackLanguage) => {
  ...
  for (const adaptationSet of period.getAll('AdaptationSet')) {
    for (const representation of adaptationSet.getAll('Representation')) {
      ...
      const baseUrls = parseBaseUrls(url, mpd, period, representation);
      ...
      if (segmentTemplate) {
        ...
        for (const type of ['initialization', 'media']) {
          ...
          if (!value.startsWith('https://')) {
            if (!baseUrls.representation)
              throw new Error(
                `Resolved Segment URL is not absolute, and no Base URL is available.`
              );
            value = new URL(value, baseUrls.representation).toString();
          }
        ...
        for (const segmentUrl of segmentUrls) {
          ...
          if (!mediaUrl) mediaUrl = baseUrls.representation;
          else if (!mediaUrl.startsWith('https://'))
            mediaUrl = new URL(mediaUrl, baseUrls.representation).toString();
        ...
        if (initialization) {
          ...
          const response = await fetch(baseUrls.representation, headers);
        ...
        segments.push({ url: baseUrls.representation, range: mediaRange });
      } else if (baseUrls.representation) {
        segments.push({ url: baseUrls.representation });
      } else {
