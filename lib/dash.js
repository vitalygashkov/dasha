...
const parseSegmentsFromTimeline = (segmentTimeline, segmentTemplate, representation) => {
  const times = [];
  let currentTime = 0;
...
const parseSegmentsFromTemplate = (
  ...
  if (segmentTimeline)
    return parseSegmentsFromTimeline(segmentTimeline, segmentTemplate, representation);
    ...
    for (const representation of adaptationSet.getAll('Representation')) {
      ...
      switch (contentType) {
        ...
        case 'audio': {
          ...
          const channels = get('AudioChannelConfiguration').get('value');
